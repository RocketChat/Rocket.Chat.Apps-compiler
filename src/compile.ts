...
import type { ICompilerDescriptor, ICompilerResult } from "./definition";
import { CompilerFileNotFoundError } from "./definition";

const {
    promises: fs,
    constants: { R_OK: READ_ACCESS },
} = require("fs");

export async function compile(
    compilerDesc: ICompilerDescriptor,
    sourceDir: string,
    outputFile: string,
    ...
    try {
        ...
        logger.error(
            `Can't read app's manifest in "${sourceAppManifest}". Are you sure there is an app there?`,
        );
    ...
    try {
        ...
        logger.debug(
            `Using TypeScript ${appTs.version} as specified in app's dependencies`,
        );
    } catch {
        logger.debug(
            "App doesn't have the typescript package as a dependency - compiler will fall back to TypeScript 2.9.2",
        );
    }
