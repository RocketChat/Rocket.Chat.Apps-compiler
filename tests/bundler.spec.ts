...
function makeResult(compiledJs: string): ICompilerResult {
    const file = { name: 'MyApp.js', content: '', version: 1, compiled: compiledJs };
    return {
        files: { 'MyApp.js': file },
        mainFile: file,
        implemented: [],
...
`;
        const { bundle } = await bundleCompilation(makeResult(compiledJs), stubValidator);

        expect(bundle).to.match(/require\(["']@rocket\.chat\/apps-engine\/definition\/IApp["']\)/);
    });
...
`;
        ...
        expect(bundle).to.match(/require\(["']@rocket\.chat\/apps-engine\/definition\/metadata["']\)/);
    });
...
`;
        ...
        expect(bundle).to.match(/require\(["']@rocket\.chat\/apps-engine\/definition\/App["']\)/);
        // A bundled apps-engine would introduce its own internal requires; none should appear
        expect(bundle).to.not.match(/require\(["']@rocket\.chat\/apps-engine\/definition\/[^"']+["']\).*require\(["']@rocket\.chat\/apps-engine/s);
    });
});
